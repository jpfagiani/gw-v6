...
NAT
    ...
    gen_nftables_conf() {
        ...
        printf '        # Painel Web (porta 5000)\n'
        printf '        ip saddr %s tcp dport 5000 ct state new accept\n'         "$NET_INT"
        ...
        printf '        iif %s oif %s ip daddr { %s, %s, %s, %s, %s } tcp dport 53 accept\n' \
            "$LAN_IFACE" "$WAN_IFACE" "$DNS1" "$DNS2" "$DNS3" "$DNS4" "$DNS5"
        printf '        # LAN -> internet: apenas ips_livres têm acesso direto\n'
        printf '        # Demais clientes devem usar proxy (porta %s)\n'              "$PROXY_PORT"
...
# Forçar Squid a usar IP da WAN para intranet gov SP (cartoriosap, policiapenal)
# Sem isso o Squid conecta com IP da LAN (192.168.x.x) e o servidor recusa
