...
function FindProxyForURL(url, host) {
    // Loopback e nomes simples — direto
    ...
    // Intranet 10.0.0.0/8 — DIRETO (preserva NAT 1:1 de cada cliente)
    // Sites gov SP (cartoriosap, policiapenal etc.) resolvem para 10.200.x.x
    // O NAT 1:1 de cada cliente deve ser preservado — não passar pelo Squid
    if (isInNet(host, "10.0.0.0",      "255.0.0.0"))     return "DIRECT";

    // Domínios governamentais e intranet — direto pelo hostname
    if (dnsDomainIs(host, ".gov.br"))                    return "DIRECT";
    if (dnsDomainIs(host, ".sp.gov.br"))                 return "DIRECT";
    if (dnsDomainIs(host, ".jus.br"))                    return "DIRECT";
    if (dnsDomainIs(host, ".tjsp.jus.br"))               return "DIRECT";
    if (dnsDomainIs(host, ".mp.br"))                     return "DIRECT";
    if (dnsDomainIs(host, ".def.br"))                    return "DIRECT";
    if (dnsDomainIs(host, ".leg.br"))                    return "DIRECT";
    if (dnsDomainIs(host, ".cartoriosap.sp.gov.br"))     return "DIRECT";
    if (dnsDomainIs(host, ".policiapenal.sp.gov.br"))    return "DIRECT";

    // Verificar se o IP resolvido está na intranet 10.x.x.x
    var resolved = dnsResolve(host);
    if (resolved && isInNet(resolved, "10.0.0.0", "255.0.0.0")) return "DIRECT";
